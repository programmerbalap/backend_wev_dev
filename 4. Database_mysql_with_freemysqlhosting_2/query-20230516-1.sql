...
SELECT p.name AS NamaPlaylist, u.name AS NamaUser , t.title AS JudulTrack , s.name AS NamaSinger, a.name AS NamaAlbum
FROM playlist_track pt
INNER JOIN playlist p ON p.playlist_id = pt.playlist_id
INNER JOIN user u ON u.user_id = p.user_id
INNER JOIN track t ON t.track_id = pt.track_id
INNER JOIN album a ON a.album_id = t.album
INNER JOIN singer s ON s.singer_id = t.singer
WHERE u.name = "subhan"
ORDER BY p.playlist_id;

-- Jumlah data dalam table
SELECT COUNT(*) FROM playlist_track pt
INNER JOIN playlist p ON pt.playlist_id = p.playlist_id
WHERE p.name = "lagu band" ;